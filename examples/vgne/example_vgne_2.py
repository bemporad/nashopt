...
# gnep = GNEP(sizes, f=f, g=g, ng=ncon, lb=lb, ub=ub, Aeq=Aeq, beq=beq, variational=True) # equivalent GNEP
gnep = GNEP(sizes, f=f, g=g, ng=ncon, lb=lb,
            ub=ub, h=h, nh=1, variational=True)
...
for i in range(gnep.N):
    x_br_vgne, fbr_opt_vgne, iters_vgne = gnep.best_response(
        i, x_star_vgne, rho=1.e8)
