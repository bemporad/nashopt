...
for alpha1 in alpha1_set:
    sol = pgnep.solve(J=J, rho=1.e4, alpha1=alpha1,
...
for k in k_range:
    ...
    nnz = jnp.sum(jnp.abs(x_star) > 1e-6)
    print(f"N = {N:3d}, CPU time = {stats.elapsed_time: 7.4f} s, ||x*||_0 = {nnz:3d}, J(x*) = {J_star:7.4f}, ||R(x*)||_2 = {jnp.linalg.norm(residual):7.2g}")
